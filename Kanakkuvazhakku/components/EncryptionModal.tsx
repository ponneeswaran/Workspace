    ...
    <Modal visible={isOpen} transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.backdrop}>
        <View style={styles.sheet}>
          ...
          <TextInput placeholder={t('Enter password (optional)')} value={password} onChangeText={setPassword} secureTextEntry style={styles.input} />
          <View style={styles.row}>
            <TouchableOpacity onPress={onClose} style={styles.btnSecondary}><Text>{t('Cancel')}</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => { onConfirm(password || undefined); setPassword(''); }} style={styles.btnPrimary}><Text style={styles.confirmText}>{t('Confirm') || 'Confirm'}</Text></TouchableOpacity>
          </View>
        </View>
      </View>
    </Modal>
